...
class LocalGroup{
    public :
        ...
        void sort(vector<solution> &spider_monkeys, vector<double> &fitness);
    private :
...
void inline LocalGroup::sort(vector<solution> &spider_monkeys, vector<double> &fitness){

    for(int i=from+1 ; i<end ; i++){
        for(int j=i ; j>from ; j--){
            if(fitness[j] > fitness[j-1]){
                swap(spider_monkeys[j], spider_monkeys[j-1]);
...
{
public:
    ASMO(int ll, int gl, int ps, double pr, bool l);
...
inline ASMO::ASMO(int ll, int gl, int ps, double p_r, bool l){
    local_limit = ll;
    global_limit = gl;
    population_size = ps;
    pr = p_r;
...
int inline ASMO::partnerSelect(LocalGroup &lg, int idx){
    int select = idx;
    while(select == idx)
...
double inline ASMO::calculateFitness(double funv){
    if (funv >= 0)
        return 1 / (funv + 1);
    else
        return 1 + fabs(funv);
...
void inline ASMO::init(double ub, double lb, int d, string objf){
...
void inline ASMO::globalLearning(){
...
void inline ASMO::globalPhase(int d, string objf, double ub, double lb){
...
void inline ASMO::localPhase(int d, string objf, double ub, double lb){
...
void inline ASMO::localDecision(int d, double ub, double lb){
...
void inline ASMO::globalDecision(double ub, double lb){
...
/* run ASMO by "func" as optimization object
 *
 * @ epoch : run times
 ...
 *  
 */
double inline ASMO::run(string objf, int d, int r, int evals, unordered_map<int, vector<double>> &record){
